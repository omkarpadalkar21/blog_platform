...
/**
 * Implementation of authentication service that handles user authentication and
 * JWT token management. This service provides methods for authenticating users,
 * generating JWT tokens, and validating tokens.
 */
@Service
...
public class AuthServiceImpl implements AuthService {
     ...
     *
     * This method uses Spring Security's AuthenticationManager to verify the
     * user's credentials. If authentication is successful, it returns the
     * UserDetails object containing user information. If authentication fails,
     * Spring Security will throw an AuthenticationException.
     *
     ...
     *
     * This method creates a JSON Web Token containing user information and
     * signs it with HMAC SHA-256. The token includes the user's username as the
     * subject, issue time, and expiration time. The token expires after 24
     ...
     *
     * This method extracts the username from the token, validates the token's
     * signature, and returns the UserDetails object for the user. If the token
     * is invalid or expired, JWT parsing will throw an exception.
     *
     ...
     *
     * This private method parses the JWT token, verifies its signature, and
     * extracts the subject claim which contains the user's email address.
     *
